...
void compute_disparity(uint8_t* left, uint8_t* right, uint8_t* disparity, int width, int height) {
...
#define WIDTH (60)
#define HEIGHT (60)
#define IMG_MEM_BASE_ADDR ((uint8_t*)0xFFFF)

//static uint8_t left_bmp [WIDTH * HEIGHT * 3];
//static uint8_t right_bmp [WIDTH * HEIGHT * 3];
//static uint8_t left [WIDTH * HEIGHT];
//static uint8_t right [WIDTH * HEIGHT];
//uint8_t disparity [WIDTH * HEIGHT];
//uint8_t filtered [WIDTH * HEIGHT];

int main() {
	uint8_t* left_bmp 	= IMG_MEM_BASE_ADDR;
	uint8_t* right_bmp 	= IMG_MEM_BASE_ADDR + WIDTH*HEIGHT*3;
	uint8_t* left 		= IMG_MEM_BASE_ADDR + 2*WIDTH*HEIGHT*3;
	uint8_t* right 		= IMG_MEM_BASE_ADDR + 2*WIDTH*HEIGHT*3 + WIDTH*HEIGHT;
	uint8_t* disparity 	= IMG_MEM_BASE_ADDR + 2*WIDTH*HEIGHT*3 + 2*WIDTH*HEIGHT;
	uint8_t* filtered 	= IMG_MEM_BASE_ADDR + 2*WIDTH*HEIGHT*3 + 3*WIDTH*HEIGHT;
	xil_printf("Started\n");
    ...
    compute_disparity(left, right, disparity, width, height);
